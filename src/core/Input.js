export class Input {
  ...
  bindKeyboard(){const map={ArrowLeft:'left',KeyA:'left',ArrowRight:'right',KeyD:'right',ArrowUp:'jump',KeyW:'jump',Space:'jump',ShiftLeft:'focus',ShiftRight:'focus',KeyE:'interact'};addEventListener('keydown',e=>{const key=map[e.code];if(e.target?.closest?.('input,button,[role=dialog]')&&!e.target?.closest?.('[data-input]'))return;if(key){e.preventDefault();if(!this.state[key])this.pressed.add(key);this.state[key]=true}});addEventListener('keyup',e=>{const key=map[e.code];if(key)this.state[key]=false});addEventListener('blur',()=>this.reset())}
